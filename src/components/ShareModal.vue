  ...
  <div v-if="isActive" class="share-modal-overlay" @click="closeModal">
    <div class="share-modal" @click.stop>
      ...
      <div class="modal-actions">
        <button class="action-btn telegram-btn" @click="shareOnTelegram">
...
function shareOnTelegram() {
  const guestNames = capturedGuests.value
    .slice(0, 5)
    .map((g) => (g.name || '').toString().trim())
    .filter(Boolean);

  const guestsLine = guestNames.length > 0
    ? 'Поймал: ' + guestNames.join(', ') + (capturedGuests.value.length > 5 ? '…' : '')
    : '';
...
.telegram-btn:hover {
  border-color: #1c87b8;
  box-shadow: 0 6px 0 rgba(0, 0, 0, 0.3), 0 0 20px rgba(34, 158, 217, 0.4);
}
